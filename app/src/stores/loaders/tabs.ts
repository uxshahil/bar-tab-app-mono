...
export const useTabsStore = defineStore('tabs-store', () => {
  ...
  const getOpenTabs = async (force = false) => {
    ...
    if (data) {
      // Helper to merge totals by manually summing items is now extracted to `mergeOpenTabTotals` function below
      openTabsList.value = (await mergeOpenTabTotals(data as Tabs)) as Tabs
    }
  }

  // Extracted helper function for getOpenTabs to reduce cognitive complexity
  const mergeOpenTabTotals = async (rawTabs: Tabs) => {
    if (!rawTabs || rawTabs.length === 0) return rawTabs

    const tabIds = rawTabs.map((t: Tabs[number]) => t.id)
  ...
  const createPayment = async (paymentData: {
    ...
    const { data, error, status } = await createTabPaymentQuery(paymentData as TabPaymentInsert)
  ...
  const _updateDrinkInTab = async (params: {
    ...
    // Extracted optimistic update logic
    const applyLocalDrinkUpdate = () => {
      ...
      if (quantityChanged && tab.value?.id === tabId) {
        const diff = newItemTotal - oldItemTotal
        tab.value.total_before_tip = (tab.value.total_before_tip || 0) + diff
        tab.value.tax_amount =
          tab.value.total_before_tip * (Number(import.meta.env.VITE_VAT) || 0.15)
      ...
      return { newItemTotal, quantityChanged, oldItemTotal }
    }

    const revertLocalDrinkUpdate = () => {
    ...
    const { newItemTotal, quantityChanged, oldItemTotal } = applyLocalDrinkUpdate()
