...
const currentViewTotals = computed(() => {
  ...
  const totalOwed = totals.totalBeforeTip + (selectedSplitId.value === null ? (Number(tab.value?.tip_amount) || 0) : 0)
...
<template>
  <div v-if="tab" class="space-y-6 p-4">
    ...
    <div class="border-t pt-4 space-y-2">
      ...
      <div v-if="selectedSplitId === null && (tab.total_paid || 0) > 0" class="flex justify-between text-muted-foreground text-sm mt-1">
        <span>Paid:</span>
        <span>-{{ formatCurrency(tab.total_paid || 0) }}</span>
      </div>

      <div v-if="selectedSplitId === null && (tab.total_paid || 0) > 0" class="flex justify-between font-bold text-lg text-primary mt-1 border-t border-dashed pt-2">
