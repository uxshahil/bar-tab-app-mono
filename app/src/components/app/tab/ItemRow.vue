...
const emit = defineEmits<{
  (e: 'startEditing', item: TabItem): void
  (e: 'cancelEditing'): void
  (e: 'saveEdit', item: TabItem, form: { quantity: number; specialInstructions: string }): void
  (e: 'toggleNote', itemId: number): void
  (e: 'updateQuantity', item: TabItem, change: number): void
}>()

const localEditForm = reactive({ quantity: 1, specialInstructions: '' })

watch(
  () => props.editingItemId === props.item.id,
  (isEditing) => {
    if (isEditing) {
      localEditForm.quantity = Number(props.item.quantity)
      localEditForm.specialInstructions = props.item.special_instructions || ''
    }
  },
  { immediate: true },
)
  ...
  <div v-else class="bg-muted/50 -mx-1 px-2 py-2 rounded-md space-y-2">
    <div class="flex items-center justify-between mb-2">
      ...
      <div class="flex gap-1">
        ...
        <Button size="icon" class="h-6 w-6" @click.stop="emit('saveEdit', item, localEditForm)">
    ...
    <div class="flex items-center gap-2">
      <div class="flex items-center border rounded-md bg-background h-8">
        <button
          class="px-2 hover:bg-muted h-full border-r"
          @click.stop="localEditForm.quantity = Math.max(1, localEditForm.quantity - 1)"
        >
          -
        </button>
        <span class="w-8 text-center font-bold text-sm">{{ localEditForm.quantity }}</span>
        <button class="px-2 hover:bg-muted h-full border-l" @click.stop="localEditForm.quantity++">
          +
        </button>
      </div>
      <input
        v-model="localEditForm.specialInstructions"
        ...
        @keydown.enter="emit('saveEdit', item, localEditForm)"
        @click.stop
      />
    </div>
  </div>
