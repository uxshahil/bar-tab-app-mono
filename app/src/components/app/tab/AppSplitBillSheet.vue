...
const createNewSplit = async () => {
  loading.value = true
  const maxSplitNumber =
    tabSplits.value?.reduce((max, s) => Math.max(max, s.split_number || 0), 0) || 0
...
const deleteSplit = async (splitId: number) => {
  if (!confirm(`Are you sure you want to delete Split? Items will be returned to unassigned.`))
    return
  ...
  <Sheet v-model:open="isOpen">
    <SheetContent class="w-[400px] sm:w-[600px] overflow-y-auto max-h-screen px-4">
      <SheetHeader>
        ...
        <SheetDescription> Assign items to different splits. </SheetDescription>
      </SheetHeader>
        ...
        <div class="space-y-4">
          ...
          <div v-if="tabItems?.length" class="space-y-2">
            ...
            >
              ...
              <div class="w-[140px]">
                <Select
                  :model-value="getSplitForItem(item.id)"
                  @update:model-value="(val: any) => assignItemToSplit(item.id, val)"
                >
                  <SelectTrigger>
          ...
          <div v-else class="text-center py-8 text-muted-foreground">No items to split.</div>
        </div>
      </div>
    </SheetContent>
  </Sheet>
