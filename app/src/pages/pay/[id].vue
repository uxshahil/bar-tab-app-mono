...
const tabId = computed(() => {
  ...
  const paramId = (route.params as Record<string, string>).id
...
<template>
  <div class="min-h-screen bg-background flex flex-col items-center p-4 md:p-8">
    ...
    <div
      v-else-if="success"
      class="flex-1 flex flex-col justify-center items-center space-y-6 text-center animate-in fade-in zoom-in duration-500 max-w-md w-full"
    >
      <div
        class="h-24 w-24 rounded-full bg-green-100 flex items-center justify-center text-green-600"
      >
        ...
        <div class="md:col-span-7 space-y-6 flex flex-col justify-center mb-8">
          ...
          <div class="bg-card border rounded-xl p-6 shadow-sm space-y-4">
            <h3 class="font-semibold text-lg border-b pb-3 flex justify-between">
              ...
              <span
                v-if="selectedSplitId"
                class="text-xs font-normal text-muted-foreground bg-secondary px-2 py-1 rounded-md"
              >
            ...
            <div class="space-y-3">
              ...
              <div
                v-if="!selectedSplitId && (totalPaid || 0) > 0"
