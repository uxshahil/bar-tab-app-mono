...
export const fetchDrinks = (search?: string) => {
  let q = supabase.from('menu_item').select('*, category (name, slug, menu (name, slug))')
...
export const drinksByCategoryQuery = (categorySlug: string) =>
  supabase
    .from('menu_item')
...
export const updateDrinkQuery = (id: number, updatedDrink = {}) =>
  supabase.from('menu_item').update(updatedDrink).eq('id', id)

export const deleteDrinkQuery = (id: number) => supabase.from('menu_item').delete().eq('id', id)
